...
public class Counter <E> implements Serializable {
  ...
  private double cacheTotalCount = 0.0;
  ...
  /**
   * Finds the key with minimum count.  This is a linear operation, and ties are
   * broken arbitrarily.
   *
   * @return a key with minimum count
   */
  public E argMin() {
  ...
  public E sample(){
	  if(Math.abs(totalCount()-1.0) > 1e-4){
		  System.out.println("WARNING: sampling from non-normalized counter: " + totalCount());
	  }
	  ...
	  while(rolling <= rand){
		  cand = iter.next();
		  double count = getCount(cand);
		  rolling += count;
	  }
	  return cand;
  }
  
  public void setTo(Counter <E> other){
	  //(clone counter)
	  this.entries = other.entries;
	  this.currentModCount = other.currentModCount;
	  this.cacheTotalCount = other.cacheTotalCount;
